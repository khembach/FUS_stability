...
## 1
s <- "8h_up"
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 10)[-c(1, 3:5, 7:8),], 
                 topGO(ora_res[[s]], ontology = "BP", number = 8)[-c(2,4:7),],
                 topGO(ora_res[[s]], ontology = "MF", number = 1))
p <- ora_circle_plot(df_comb, repel_xlim = c(2.5, NA), box.padding = 0.8)
p
...
## 2, 3
s <- "8h_down"
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 2), 
                 topGO(ora_res[[s]], ontology = "BP", number = 3)[-2,],
                 topGO(ora_res[[s]], ontology = "MF", number = 3)[-1,])
...
## 1, 3
s <- "12h_up"
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 6)[-2,], 
                 topGO(ora_res[[s]], ontology = "BP", number = 9)[-c(1:2, 5, 7:8),],
                 topGO(ora_res[[s]], ontology = "MF", number = 3)[-2,])
p <- ora_circle_plot(df_comb, repel_xlim = NA, box.padding = 0.8)
p
ggsave(file = here("analysis", "figures", "final", paste0("ora_res_", s, ".png")), 
       plot = p, width = 6.5, height = 7, dpi = 500, type = "cairo")
...
## 1, 2, 3, 5, 7
s <- "12h_down"
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 7)[-c(3:5),], 
                 topGO(ora_res[[s]], ontology = "BP", number = 9)[-c(3:4, 6:8),],
...
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 1), 
                 topGO(ora_res[[s]], ontology = "BP", number = 3),
                 topGO(ora_res[[s]], ontology = "MF", number = 9)[-c(2:8),])
...
## 1
s <- "24h_down"
df_comb <- rbind(topGO(ora_res[[s]], ontology = "CC", number = 8)[-c(2:4, 7),], 
                 topGO(ora_res[[s]], ontology = "BP", number = 8)[-c(2:4, 7),],
                 topGO(ora_res[[s]], ontology = "MF", number = 1))
p <- ora_circle_plot(df_comb, repel_xlim = NA, box.padding = 0.8)
p
...
## For timepoint 8h: contingency table
##          DE    noDE    total
##  FUS     9     298     307
##  noFUS   717   14063   14780
##  total   726   14361   15087

no_fus <- length(expr_sns) - length(gene_list[["8h"]])
# no_DE_fus <- length(gene_list[["FUS"]])
15087 - 726
## FUS tarets
gene_list[["FUS"]] %in% gene_list[["8h"]] %>% table
## no FUS
length(expr_sns)  - length(gene_list[["FUS"]])
14780-717
fisher.test(matrix(c(9, 298, 717, 14063), nrow = 2, byrow = TRUE), 
            alternative = "greater")
## 12h
length(expr_sns) - length(gene_list[["12h"]])
gene_list[["FUS"]] %in% gene_list[["12h"]] %>% table

14780-4061
fisher.test(matrix(c(76, 231, 4061, 10719), nrow = 2, byrow = TRUE), 
            alternative = "greater")
## 24h
length(expr_sns) - length(gene_list[["24h"]])
gene_list[["FUS"]] %in% gene_list[["24h"]] %>% table
14780-460
fisher.test(matrix(c(6, 301, 460, 14320), nrow = 2, byrow = TRUE), 
            alternative = "greater")
```
...
for (n in names(res_tables)) {
  ...
  p <- ggplot(d, aes(x = logFC.SNS,  y = logFC.stab)) + 
    ...
    theme_bw(base_size = 20) + ggtitle(n) +
    theme(aspect.ratio = 1) +
    scale_x_continuous(limits = x_range, name = "logFC(KI-WT) SNS 6-month-old") + 
    scale_y_continuous(limits = x_range, "logFC(KI-WT) actinomycin D treated")
  print(p)
...
## ANOVA is affected by unqual sample sizes and that is the case here, 
